...
describe("deleteSchemaVersions", () => {
  ...
  optionalDescribe(supported)("should delete schema versions", () => {
  ...
  describe("should handle errors", () => {
    test("error handling for non-existent versions", async () => {
      ...
      // Try to delete non-existent versions - should throw an error
      await expect(
        client.deleteSchemaVersions(schemaName, [99, 100])
      ).rejects.toThrow(/does not have versions/);
    });
    ...
    test("partial failures in error list", async () => {
      ...
      // Delete mix of existing and non-existing versions - should throw an error
      await expect(
        client.deleteSchemaVersions(schemaName, [1, 999])
      ).rejects.toThrow(/does not have versions/);
    });
  });
});
